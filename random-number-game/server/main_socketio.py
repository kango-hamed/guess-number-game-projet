...
app = FastAPI(title="GuessCraft API", version="1.0.0")
...
async def make_guess(sid, data):
    ...
    if room.status == "finished":
        await sio.emit('game_finished', {
            ...
            'target_number': room.target_number
        }, room=player.room_id)
...
if __name__ == "__main__":
    import uvicorn
    print("=" * 50)
    print("  SERVEUR GUESSCRAFT - SOCKET.IO")
    print("=" * 50)
    print("Serveur sur http://127.0.0.1:8000")
    print("Socket.IO sur http://127.0.0.1:8000/socket.io/")
    print("=" * 50)
    uvicorn.run("main_socketio:socket_app", host="0.0.0.0", port=8000, reload=True)
